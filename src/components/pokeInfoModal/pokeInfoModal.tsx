...
export function PokeInfoModal({isOpen, onRequestClose}:PokeInfoModalProps){
    const {selectedPokemon, pokemonInfo, capitalizeFirstLetter, getTypeRelations} =usePokemonsInfo();
    const {GetPokemonImg} = usePokemonsImg();
    const [checked, setChecked] = useState(false);

    let teste = getTypeRelations()
    ...
    return(
        ...
        <Container onAuxClick={(e)=>{ if(e.button!==2){e.preventDefault(); onRequestClose()}}}>
            ...
            <PokemonCard types={pokemonInfo.types[0].name}>
            <PokemonInfo/>
            <StatsBars>
